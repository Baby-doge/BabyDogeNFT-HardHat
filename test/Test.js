...
it("Should deploy the NFT Contract", async function () {
  ...
  babyDogeNft = await BabyDogeNft.deploy(
    "BabyDogeNFT",
    "BDNFT",
    "https://base",
    "200"
...
it("should not allow anyone who is not whitelisted to presale Mint", async function () {
  ...
  await expect(
    babyDogeNft.connect(account8).mintWhitelistDoge("1", hexProof, overrides)
  ).to.be.reverted;
});
...
it("should allow anyone who is whitelisted to presale Mint", async function () {
  ...
  await babyDogeNft
    .connect(account7)
    .mintWhitelistDoge("1", hexProof, overrides);
...
it("should allow anyone to Mint", async function () {
  ...
  console.log(
    "Balance of babydoge",
    await babyDogeNft.balanceOf(account8.address)
  );
...
it("should not allow anyone to Mint more the the limit", async function () {
  ...
  await expect(babyDogeNft.connect(account8).mintDoge("1", overrides)).to.be
    .reverted;
  let totalsupply = await babyDogeNft.totalSupply();
  expect(totalsupply).to.equal("103");
});
...
it("should allow owner to withdraw", async function () {
  ...
  expect(parseInt(ownerBalance)).to.be.lessThan(parseInt(ownerBalanceAfter));
});
...
it("should allow transfers", async function () {
  ...
  await babyDogeNft
    .connect(opensea)
    .transferFrom(account9.address, account10.address, "103");
...
it("should allow owner transfers second time", async function () {
  ...
  await babyDogeNft
    .connect(opensea)
    .transferFrom(owner.address, account10.address, "0");
...
it("should not allow false transfers", async function () {
  ...
  await expect(
    babyDogeNft
      .connect(opensea)
      .transferFrom(owner.address, account9.address, "103")
  ).to.be.reverted;
  expect(await babyDogeNft.balanceOf(account10.address)).to.equal(2);
  expect(await babyDogeNft.ownerOf("103")).to.equal(account10.address);
});
